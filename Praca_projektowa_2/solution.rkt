...
(define (sim-wait! simulation time)
  (if (<= time 0)
      (void)
      (begin
        (set-sim-actual_time! simulation (+ 0.00005 (sim-time simulation)))
        (remove-events simulation (sim-time simulation))
        (sim-wait! simulation (- time 0.00005)))))
...
(define (wire_event_dual name output input_1 input_2 function)
  (if (and (equal? (wire-owner input_1) (wire-owner input_2))
           (equal? (wire-owner input_1) (wire-owner output)))
      (let ([event (wire_action name output input_1 input_2 function)])
        (begin
          ...
          (wire-set! output (function (wire-value input_1) (wire-value input_2)))))
      (error "DIFFRENT SIMULATION")))

(define (wire_event_unary name output input_1 function)
  (if (equal? (wire-owner input_1) (wire-owner output))
      (let ([event (wire_action name output input_1 input_1 function)])
        (begin
          (set-wire-actions! input_1 (cons event (wire-actions input_1)))
          (wire-set! output (function (wire-value input_1) (wire-value input_1)))))
      (error "DIFFRENT SIMULATION")))
...
(define (gate-not output input)
  (wire_event_unary "not" output input (λ(x y) (not x))))
...
(define wire_A0 (make-wire make_true))
(define wire_A1 (make-wire make_true))
(define wire_A2 (make-wire make_true))
(define wire_A3 (make-wire make_true))
(define wire_A4 (make-wire make_true))
(define wire_A5 (make-wire make_true))
(define wire_B0 (wire-not wire_A2))
(define wire_B1 (wire-and wire_A1 wire_B0))
(define wire_B2 (wire-and wire_A3 wire_A4))
(define wire_B3 (wire-and wire_A0 wire_B1))
(define wire_B4 (wire-and wire_B0 wire_B2))
(define wire_B5 (wire-and wire_B2 wire_A5))
(define wire_B6 (wire-not wire_B3))
(define wire_B7 (wire-and wire_B1 wire_B6))
(define wire_B8 (wire-and wire_B4 wire_B5))
(define wire_output (wire-and wire_B7 wire_B8))
...
(define make_true2 (make-sim))
(define wire_C0 (make-wire make_true2))
(define wire_C1 (make-wire make_true2))
(define wire_C2 (make-wire make_true2))
(define wire_C3 (make-wire make_true2))
(define wire_C4 (make-wire make_true2))
(define wire_C5 (make-wire make_true2))
(define wire_D0 (wire-and wire_C0 wire_C1))
(define wire_D1 (wire-and wire_C2 wire_C3))
(define wire_D2 (wire-or wire_C4 wire_C5))
(define wire_D3 (wire-not wire_D1))
(define wire_D4 (wire-xor wire_D0 wire_D3))
(define wire_D5 (wire-xor wire_D1 wire_D2))
(define wire_D6 (wire-not wire_D4))
(define wire_D7 (wire-not wire_D5))
(define wire_D8 (wire-and wire_D6 wire_D7))
(define wire_output_2 (wire-and wire_D0 wire_D8))
(sim-wait! make_true2 20)
(check-equal? (wire-value wire_output_2) #f)
(wire-set! wire_C0 #t)
(wire-set! wire_C1 #t)
(sim-wait! make_true2 20)
(check-equal? (wire-value wire_output_2) #t)
|#